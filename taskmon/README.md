...
- **`k` 结束整组进程**（按 PID 快照 + 硬保护名单 + 二次确认，见「结束进程与安全设计」）
...
`k` 对光标所在分组发起结束，执行链路刻意做保守：

- **按 PID 快照执行**：确认时冻结自动刷新并捕获该组当前全部 PID，`taskkill /F /T /PID` **串行**逐个结束（`/T` 连带子进程树；单个 PID 树终止超时 15s 时自动回退 `/F` 直接结束，保证组成员必死）；快照之后新起的同名进程不会被误伤。串行而非并行：并行 `/T` 会对互相重叠的进程树反复快照互相拖慢，深树场景（如 IDE 带 node 子进程链）曾导致整体超时一刀未落
- **二次确认**：确认条显示组名、进程数与合计内存，`y` 执行、`n`/`Esc`/`q` 取消，其他按键忽略；执行中状态条显示进度 `(i/N) · PID`
...
- **占比列**：组行 = 组总内存 / 物理总量（因工作集虚高，各组占比之和可能 >100%）；成员行 = 单进程 / 组总量。
