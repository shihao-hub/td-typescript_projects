...
| `Enter`/`空格` | 切换折叠 |
| `a` | 全收起 ↔ 全展开 |
| `c` | 进入复制视图：主屏打印当前 JSON Pointer、选中行 key 和子树，长值不截断；`Enter` 返回 TUI，`q` 退出 jtree |
| `q`/`Ctrl+C` | 退出（退出码 0） |

复制视图退回主屏，方便用终端原生选择复制；返回备用屏后保留展开状态和光标位置。
...
- TUI 和纯打印单行超出终端宽度按显示宽度截断加 `…`（中文等宽字符按 2 列计）；复制视图用 `truncate: false` 输出完整值
